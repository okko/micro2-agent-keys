...
import { STATES, INTEGRATION_SLOT_COUNT, normalizeState } from './states.js';
import { type VSCodeIntegration, type VSCodeSlot } from './vscode.js';
...
async function handle(api: DaemonApi, req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (req.method === 'POST' && match) {
    ...
    if (!Number.isInteger(index) || index < 0 || index >= INTEGRATION_SLOT_COUNT) {
      return send(res, 400, { error: `slot must be 0..${INTEGRATION_SLOT_COUNT - 1}` });
    }
